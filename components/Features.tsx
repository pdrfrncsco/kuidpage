      ...
      <div className="container mx-auto px-6">
        <div className="flex flex-col lg:flex-row items-center gap-20">
          <div className="flex-1">
            <h2 className="text-4xl lg:text-6xl font-black text-gray-900 dark:text-slate-50 mb-10 tracking-tighter leading-none">
              Poderosa por dentro, <br />
              <span className="text-gradient dark:drop-shadow-[0_0_18px_rgba(0,209,255,0.22)]">Simples por fora.</span>
            </h2>
            <div className="grid sm:grid-cols-2 gap-4">
              {features.map((feature, i) => (
                <div key={i} className="flex items-center gap-4 p-5 rounded-2xl bg-white dark:bg-white/[0.04] border border-gray-200 dark:border-white/10 hover:border-kuid-green/20 transition-all shadow-sm dark:shadow-none">
                  <div className="text-kuid-green bg-kuid-green/10 p-3 rounded-xl">
                    {icons[i]}
                  </div>
                  <span className="text-gray-700 dark:text-slate-200 font-bold text-sm tracking-tight">{feature}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 p-1 bg-kuid-gradient rounded-[48px] shadow-2xl shadow-kuid-blue/20">
            <div className="bg-deep-navy rounded-[47px] p-12 h-full flex flex-col justify-center relative overflow-hidden">
               ...
               <h3 className="text-3xl font-black text-slate-50 mb-8 leading-tight tracking-tight">Infraestutura preparada para integrações institucional.</h3>
               <p className="text-slate-300 mb-10 leading-relaxed text-lg font-medium">
